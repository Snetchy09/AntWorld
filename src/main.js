...
function updateAnt(ant,dt){
 ...
 else if(ant.action==='dig'){ ant.digCooldown-=dt; ax=ant.dir*18; if(ant.digCooldown<=0){ const digX=ant.x+ant.dir*20, digY=ant.y+20; carve(digX, digY, 34); terrain.builds.push({x:ant.x-ant.dir*16,y:ant.y+13,r:6+rand()*5,life:1}); ant.digCooldown=.55; ant.energy-=2.6; }}
...
function nextRun(){ const survivors=ants.sort((a,b)=>b.fitness-a.fitness).slice(0,10); state.run++; state.time=0; terrain.digs.length=0; terrain.builds.length=0; resources.splice(0,resources.length,...generateResources()); ants=createPopulation(34,survivors.length?survivors:null); state.selected=null; }
...
function tick(t){ const dt=Math.min(.04,(t-state.last)/1000||0); state.last=t; const move=420*dt; if(state.keys.has('arrowleft')||state.keys.has('a'))state.camera.tx-=move; if(state.keys.has('arrowright')||state.keys.has('d'))state.camera.tx+=move; if(state.keys.has('arrowup')||state.keys.has('w'))state.camera.ty-=move; if(state.keys.has('arrowdown')||state.keys.has('s'))state.camera.ty+=move; state.camera.tx=clamp(state.camera.tx,0,WORLD.width-innerWidth); state.camera.ty=clamp(state.camera.ty,0,WORLD.height-innerHeight); state.camera.x=lerp(state.camera.x,state.camera.tx,.08); state.camera.y=lerp(state.camera.y,state.camera.ty,.08); if(!state.paused){ for(let i=0;i<state.speed;i++){ ants.forEach(a=>updateAnt(a,dt)); updatePredators(dt); state.time+=dt; autoAdvanceIfExtinct(); } } draw(); updateHud(); requestAnimationFrame(tick); }
